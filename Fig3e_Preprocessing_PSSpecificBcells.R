...
#this will be equal to the number of samples that were exported
sample <- do.call(c, lapply(strsplit(files, 'export_'), function(i)i[2])) #for each file take the part after the 2nd '_' (dash) -we want the donor name-
sample2 <- do.call(c, lapply(strsplit(sample, c('_B cells')), function(i)i[1])) #remove everything after _Unmixed
...
#take only 10,000 of the negative cells randomly and all of the positive ones:
down = 10000
...
if(sum(counts[,2]) == nrow(dat2)){print(paste0(vol, ' ok'))}
  ...
  colnames(dat5)[23] <- "DUMP" #change names of the markers so they are the same for all files
...
for(j in (1:length(split.data))) {
  ...
  write.table(one, paste0(one$Vol[1], 'specific_cells_all.csv'), sep = ',', row.names = F) 

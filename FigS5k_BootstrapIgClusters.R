# Script for the analysis of GBS datasets (elbow and Ig)
...
# path to the folde where the results will be saved
path <- '.'

# project name
file_name <- 'GBS_IgGs_SAvsEU'
...
# column for grouping (ex: 'timepoint', 'group', 'SA_EU', ...) with at most two categories
group_col <- 'SA_EU'

# group_col caregory to be used as baseline for comparisons
group_col_baseline <- 'EU'
...
ordered_cluster <- c("IgDI", "IgD.", "IgM.", "IgA.", "IgG.")
...
list_wild_boot_type <- lapply(cluster_names, function(j) {
  ...
  ps_no_intercept <- gsub(':', '', na.omit(str_extract(na.omit(str_extract(names(coef(fit)), 'PS.*')), 'PS.*:')))
  ps_intercept <- setdiff(type_names, ps_no_intercept)

  wild_boot <- lapply(c(ps_no_intercept, 'Intercept'), function(i) {
    cat('running wild bootstrap for', j, ifelse(i == 'Intercept', ps_intercept, i), '(', paste(unique(cluster_data_perc$timepoint), collapse=' x '), ')...\n')
    
    regex_string <- ifelse(i == 'Intercept', '\\(.*\\)|^timepoint.*', paste0('^type', i, '$|^type', i, ':timepoint.*', '$|^timepoint.*', '$|^\\(Intercept\\)$'))
    type_coefs <- na.omit(str_extract(names(coef(fit)), regex_string))
    if (i != 'Intercept') {
      type_weights <- c(rep(1, length(type_coefs)), rep(-1, 2))
      type_coefs <- c(type_coefs, type_coefs[1:2])
    } else {
      type_weights <- c(rep(1, length(type_coefs)), rep(-1, 1))
      type_coefs <- c(type_coefs, type_coefs[1])
    }
...
list_boxplot_type <- lapply(cluster_names, function(j) {
  ...
  mean_type <- ddply(cluster_data_perc, ~ cluster + timepoint + type, summarise, m=mean(value))
  ...
  p_boxplot_type <- ggplot(mean_type) +
    geom_boxplot(inherit.aes=FALSE, data=cluster_data_perc, show.legend=FALSE, aes(x=timepoint, y=value, group=timepoint, fill=timepoint), alpha=0.2) +
    geom_point(aes(x=timepoint, y=m, color=timepoint), alpha=0.8, size=2) +
    facet_wrap(~ type, scales='free_y') +
    theme_minimal() +
    theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.6)) +
    labs(y='Relative frequency', x='', color=paste0(group_col, ':')) +
...
p_heatmap_type <- ggplot(wild_boot_type, aes(x = cluster, y = type)) +
  ...
  labs(x = "", y = "", size = "p.adj", fill = paste0("Diff. in ", group_col, ":"))

# SAVE RESULTS -----------------------------------------------------------------

sys_time <- substr(gsub(':', '-', gsub(' ', '_', Sys.time())), 1, 19)
if (!dir.exists(paste0(path, '/outputs'))) dir.create(paste0(path, '/outputs'))
dir.create(paste0(path, '/outputs/', file_name, '_', sys_time, '_results'))
...
list_boxplot_type # named list with several boxplots for comparisons between categories of group_col

p_heatmap_type # heatmap for the difference between categories of group_col

wild_boot_type # table of results for the wild bootstrap (estimates and p-values). comparisons between categories of group_col
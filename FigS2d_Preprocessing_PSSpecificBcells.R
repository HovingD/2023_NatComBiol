...
##TO DO:
# Change wd
# Change data session info
# change files path for input
# Make sure that the ID markers are there: IgM/IgG, otherwise change to other.
# for dat2$ID, make sure that IgM and IgG are in panel, otherwise use other markers
# !!!  names <- file[2:7], this is the nr of serotype specific population that you have + empty + Bcells. change accordingly
...
# Specify the common part to be removed
common_part <- "20230714_DHSS_5L2_Revisions-Samples-"

# Iterate over each file and rename it
for (file in files) {
  # Remove the common part from the file name
  new_name <- gsub(common_part, "", file)
...
for(vol in sample3){
  ...
  dat2$ID <- paste0(dat2$CD19, '_', dat2$CD7) #create an ID of the cells based on their IgM, IgG (and IgD if needed?) values
  ...
  for(i in 2:length(file)){ #use for loop to look at each specific exported 
    ...
    if(nrow(dat3)>0){ #checks if there are any cells in there
      ...
      dat3$ID <- paste0(dat3$CD19, '_', dat3$CD7) #check for ID
  ...
  names <- file[2:6] #Make sure to adjust this to total amount of files (Bcells, empty and 5 Spn PS. We now exclude Bcells)
...
write.table(counts, '20230724_counts_specific_allCSV.csv', row.names = F, quote = F, sep = ',')

table(dat_all$PS)

dat_all2 <- dat_all[dat_all$Vol != 'Batch16_FMT',] #if there is a FMT sample this is removed. 

#Save all donors combined in one file:
write.table(dat_all2, '20230724_specific_cells_allCSV.csv', row.names = F, quote = F, sep = ',') #it save the specific cells
...
for(j in (1:length(split.data))) {
  ...
  write.table(one, paste0(one$Vol[1], '20230724_sep_specific_cells_allCSV.csv'), sep = ',', row.names = F) 

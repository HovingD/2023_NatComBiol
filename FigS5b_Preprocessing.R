...
sample0 <- do.call(c, lapply(strsplit(files, c('_')), function(i)i[1]))
sample1 <- do.call(c, lapply(strsplit(files, c('_')), function(i)i[2]))
sample2 <- paste0(sample0, "_",sample1)
sample3 <- unique(sample2)
...
tracounts[is.na(counts)] <- 0 #set msissing PS to 0 
...
for(j in (1:length(split.data))) {
  ...
  write.table(one, paste0(one$Vol[1], 'specific_cells_allCSV2.csv'), sep = ',', row.names = F) 
